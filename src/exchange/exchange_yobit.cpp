...
void Exchange_YObit::dataReceivedAuth(QByteArray data, int reqType)
{
    if (debugLevel)
...
void Exchange_YObit::depthUpdateOrder(QString symbol, double price, double amount, bool isAsk)
{
...
void Exchange_YObit::depthSubmitOrder(QString symbol, QMap<double, double>* currentMap, double priceDouble,
...
void Exchange_YObit::buy(QString symbol, double apiBtcToBuy, double apiPriceToBuy)
{
    if (tickerOnly)
        return;
    ...
    QByteArray data = "method=Trade&pair=" + pairItem.currRequestPair + "&type=buy&rate=" +
                      byteArrayFromDouble(apiPriceToBuy, pairItem.priceDecimals, 0) + "&amount=" +
                      byteArrayFromDouble(apiBtcToBuy, pairItem.currADecimals, 0) + "&";
...
void Exchange_YObit::sell(QString symbol, double apiBtcToSell, double apiPriceToSell)
{
    if (tickerOnly)
        return;
    ...
    QByteArray data = "method=Trade&pair=" + pairItem.currRequestPair + "&type=sell&rate=" +
                      byteArrayFromDouble(apiPriceToSell, pairItem.priceDecimals, 0) + "&amount=" +
                      byteArrayFromDouble(apiBtcToSell, pairItem.currADecimals, 0) + "&";
...
void Exchange_YObit::cancelOrder(QString, QByteArray order)
{
    if (tickerOnly)
        return;
