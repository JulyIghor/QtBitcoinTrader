    ...
    : Exchange()
{
    ...
    connect(this, &Exchange::threadFinished, this, &Exchange_GOCio::quitThread, Qt::DirectConnection);
}
...
void Exchange_GOCio::quitThread()
{
    clearValues();
...
void Exchange_GOCio::dataReceivedAuth(QByteArray data, int reqType)
{
    ...
    switch (reqType)
    {
        case 103: //ticker
            {
                ...
                if (!tickerSell.isEmpty())
                {
                    ...
                    if (newTickerSell == 0.0)
                        newTickerSell = tickerLastDouble;
                    ...
                    for (int n = 0; n < asksList.count(); n++)
                    {
                        ...
                        if (baseValues.groupPriceValue > 0.0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    depthSubmitOrder(baseValues.currentPair.symbol, &currentAsksMap, groupedPrice + baseValues.groupPriceValue,
                                                     groupedVolume, true);
                                    rowCounter++;
                                    groupedVolume = amount;
                    ...
                    for (int n = 0; n < bidsList.count(); n++)
                    {
                        ...
                        if (baseValues.groupPriceValue > 0.0)
                        {
                            ...
                            else
                            {
                                ...
                                if (!matchCurrentGroup || n == asksList.count() - 1)
                                {
                                    depthSubmitOrder(baseValues.currentPair.symbol, &currentBidsMap, groupedPrice - baseValues.groupPriceValue,
                                                     groupedVolume, false);
                                    rowCounter++;
                                    groupedVolume = amount;
                                    groupedPrice -= baseValues.groupPriceValue;
        ...
        case 204://orders
            {
                ...
                bool isEmptyOrders = !success && errorString == QLatin1String("no active orders");
        ...
        case 208: ///history
            {
                bool isEmptyOrders = !success && errorString == QLatin1String("no trade history");
    ...
    if (reqType >= 200 && reqType < 300)
    {
        ...
        if (!success)
        {
            ...
            if (authErrorCount > 2)
            {
                ...
                if (authErrorString == "API key not found or inactive")
...
void Exchange_GOCio::depthSubmitOrder(QString symbol, QMap<double, double>* currentMap, double priceDouble,
    ...
    switch (sendCounter)
    {
        case 0:
            if (!isReplayPending(103))
                sendToApi(103, baseValues.currentPair.currRequestPair + "/ticker/", false, true);
        ...
        case 2:
            if (!isReplayPending(109))
                sendToApi(109, baseValues.currentPair.currRequestPair + "/trades/", false, true);
...
void Exchange_GOCio::getHistory(bool force)
{
    ...
    if (!isReplayPending(208))
        sendToApi(208, "", true, true, "method=TradeHistory&since=" + lastDate + "&");
}
...
void Exchange_GOCio::buy(QString symbol, double apiBtcToBuy, double apiPriceToBuy)
{
    ...
    QByteArray data = "method=Trade&pair=" + pairItem.currRequestPair + "&type=buy&rate=" + byteArrayFromDouble(
                          apiPriceToBuy, pairItem.priceDecimals, 0) + "&amount=" + byteArrayFromDouble(apiBtcToBuy, pairItem.currADecimals,
                                  0) + "&";
...
void Exchange_GOCio::sell(QString symbol, double apiBtcToSell, double apiPriceToSell)
{
    ...
    QByteArray data = "method=Trade&pair=" + pairItem.currRequestPair + "&type=sell&rate=" + byteArrayFromDouble(
                          apiPriceToSell, pairItem.priceDecimals, 0) + "&amount=" + byteArrayFromDouble(apiBtcToSell, pairItem.currADecimals,
                                  0) + "&";
...
void Exchange_GOCio::sendToApi(int reqType, QByteArray method, bool auth, bool sendNow, QByteArray commands)
{
    ...
    if (auth)
    {
        ...
        if (sendNow)
            julyHttp->sendData(reqType, "POST /api/" + method, postData, "Sign: " + hmacSha512(getApiSign(),
                               postData).toHex() + "\r\n");
        else
            julyHttp->prepareData(reqType, "POST /api/" + method, postData, "Sign: " + hmacSha512(getApiSign(),
                                  postData).toHex() + "\r\n");
    }
    else
    {
