...
ChartsModel::ChartsModel():QObject()
{
    intervalDate=60;
    intervalCount=10;
    perfomanceStep=1;
...
void ChartsModel::addLastTrades(QList<TradesItem> *newItems)
{
    ...
    {
        if(tradesDate.count() && tradesDate.last()>newItems->at(n).date)continue;
...
/*bool ChartsModel::groupChartsData()
{
    bool error=true;
    quint32 i=0;
    quint32 chartTime=TimeSync::getTimeT()-timeOfOneItem*numberOfItems;
    for(int n=0;n<tradesData.count();n++)
    {
        if(tradesData.at(n).date>chartTime && tradesData.at(n).date<=chartTime+(i+1)*timeOfOneItem)
        {
            if(chartsData[i].first<0)
            {
                error=false;
                chartsData[i].setFirst(tradesData.at(n).price);
                chartsData[i].setLast (tradesData.at(n).price);
                chartsData[i].setMax  (tradesData.at(n).price);
                chartsData[i].setMin  (tradesData.at(n).price);
            }
            else
            {
                chartsData[i].setLast (tradesData.at(n).price);
                if(tradesData.at(n).price>chartsData[i].max)chartsData[i].setMax(tradesData.at(n).price);
                if(tradesData.at(n).price<chartsData[i].min)chartsData[i].setMin(tradesData.at(n).price);
            }
        }
        else if(tradesData.at(n).date>chartTime+(i+1)*timeOfOneItem)
        {
            n--;
            i++;
...
{
    qint16 signCount=0;
    while(step>100){step/=10;signCount++;}
    while(step<10){step*=10;signCount--;}

    if(step>50)step=50;else if(step>25)step=25;else if(step>20)step=20;else step=10;

    for(qint16 i=0;i<signCount;i++)step*=10;
    for(qint16 i=0;i<-signCount;i++)step/=10;
...
double ChartsModel::axisRound(double old_x,double old_step)
{
    ...
    while(step>1){x/=10;step/=10;signCount++;}
    while(step<0.1){x*=10;step*=10;signCount--;}

    x=qRound(x-0.5);

    for(qint16 i=0;i<signCount;i++)x*=10;
    for(qint16 i=0;i<-signCount;i++)x/=10;

    while(x+old_step<old_x)x+=old_step;
